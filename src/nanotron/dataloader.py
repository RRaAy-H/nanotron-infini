...
def get_datasets(
    ...
    Args:
        ...
        data_dir (Optional[str]): Directory containing local dataset files (for local datasets)
        data_files (Optional[Union[str, list]]): Specific files or patterns for local datasets
    ...
    if isinstance(hf_dataset_or_datasets, dict):
        ...
        for split in splits:
            ...
            if data_dir is not None or data_files is not None:
                ...
                # For local datasets, we might not need split parameter
                try:
                    ...
                    # If split doesn't work for local dataset, try without split and take all data as train
                    if split == "train":
                        raw_datasets[split] = load_dataset(
                            hf_dataset_or_datasets,
                            **load_kwargs
                        )
                        # If it's a single dataset, convert to train split
                        if hasattr(raw_datasets[split], 'train'):
                            raw_datasets[split] = raw_datasets[split]['train']
                        elif hasattr(raw_datasets[split], '__iter__') and not isinstance(raw_datasets[split], str):
                            # If it's already a dataset, keep it as is
                            pass
                    else:
                        # For other splits, skip if not available
                        continue
            else:
                # Remote HuggingFace dataset loading (existing logic)
                raw_datasets[split] = load_dataset(
                    hf_dataset_or_datasets,
