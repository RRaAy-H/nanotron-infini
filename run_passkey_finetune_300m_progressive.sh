...
CONFIG_FILE="passkey_finetune_300m_progressive_config.yaml"
...
PG19_DATA_DIR="/data1/infini-attn/infini-llama/nanotron-infini/finetuning/pg19"
if [ ! -d "$PG19_DATA_DIR" ]; then
    echo "ERROR: PG19 data directory not found: $PG19_DATA_DIR"
    exit 1
fi
echo "PG19 data found: $PG19_DATA_DIR"
...
echo "=========================================================="
echo "PROGRESSIVE PASSKEY FINETUNING FOR 300M INFINI-ATTENTION MODEL"
echo "=========================================================="
...
if [ ! -d "$CHECKPOINT_PATH" ]; then
    ...
    echo "Usage: ./run_passkey_finetune_300m_progressive.sh [checkpoint_path]"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "ERROR: Progressive finetuning failed. Check the logs above for details."
    exit 1
fi