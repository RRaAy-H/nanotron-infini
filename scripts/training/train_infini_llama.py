#!/usr/bin/env python
# filepath: /Users/zhang/Desktop/huawei/infi_llama/nanotron-infini/scripts/training/train_infini_llama.py
...
python train_infini_llama.py --config-file custom_infini_config_gpu.yaml --data-dir processed_data

# For GPU training with Flash Attention:
python train_infini_llama.py --config-file custom_infini_config_gpu.yaml --data-dir processed_data

# For GPU training without Flash Attention:
python train_infini_llama.py --config-file custom_infini_config_gpu.yaml --data-dir processed_data --disable-flash-attn

# For CPU-only training:
python train_infini_llama.py --config-file custom_infini_config_cpu.yaml --data-dir processed_data --cpu-only

# With TensorBoard integration:
python train_infini_llama.py --config-file custom_infini_config_gpu.yaml --data-dir processed_data --tensorboard-dir tensorboard_logs

# With distributed training (using torchrun):
torchrun --nproc_per_node=8 train_infini_llama.py --config-file custom_infini_config_gpu.yaml --data-dir processed_data
```
"""
...
def load_preprocessed_data(data_dir: str):
    ...
    try:
        import datasets
        dataset_path = data_path / "train_dataset"
        if not dataset_path.exists():
            raise ValueError(f"Training dataset not found in {data_dir}/train_dataset.")
...
def create_and_configure_trainer(config: Dict, args: argparse.Namespace) -> DistributedTrainer:
    """Create and configure the trainer based on the config and args."""
    # Create parallel context
    use_cpu = args.cpu_only
    if use_cpu:
        if torch.cuda.is_available() and not args.force_cpu:
            logger.warning("GPU detected but --cpu-only specified. Use --force-cpu to suppress this warning.")
        
        # CPU parallel context
        parallel_context = ParallelContext.from_torch_distributed(
            tensor_parallel_size=config["tensor_parallel_size"],
            pipeline_parallel_size=config["pipeline_parallel_size"],
            data_parallel_size=config["data_parallel_size"],
            zero1_size=config["zero1_size"],
        )
    else:
...
def main():
    ...
    # Create trainer
    trainer = create_and_configure_trainer(config, args)
    ...
    if trainer.is_main_process:
        output_dir = Path("models") / f"infini_llama_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
